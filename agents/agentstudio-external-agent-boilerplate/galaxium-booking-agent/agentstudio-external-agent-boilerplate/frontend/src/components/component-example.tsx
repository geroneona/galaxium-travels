...
export function ChatBasic(): React.JSX.Element {
  ...
  const streamResponse = (): void => {
    ...
    streamIntervalRef.current = setInterval((): void => {
      if (charIndex < fullResponse.length) {
        streamContentRef.current += fullResponse[charIndex];
        setMessages((prev): { id: number; role: string; content: string }[] =>
          prev.map((msg): { id: number; role: string; content: string } =>
    ...
    <div className="flex h-[400px] w-full flex-col overflow-hidden">
      ...
      <ChatContainerRoot className="flex-1">
        <ChatContainerContent className="space-y-4 p-4">
          {messages.map((message): React.JSX.Element => {
