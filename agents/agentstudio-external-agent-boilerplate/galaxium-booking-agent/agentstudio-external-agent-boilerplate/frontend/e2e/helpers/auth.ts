...
export async function loginIfNeeded(page: Page, interactionDelay: number = 0): Promise<void> {
  ...
  const signInBtn = page.getByRole('button', { name: /Authenticate with Keycloak/i });
  ...
  const textareaAfterLogin = page.getByPlaceholder(/Send a message to your agent/i);
  await expect(textareaAfterLogin).toBeVisible({ timeout: 10_000 });
}
